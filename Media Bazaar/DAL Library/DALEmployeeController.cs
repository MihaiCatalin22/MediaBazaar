    ...
    public class DALEmployeeController : IEmployeeController
    {
                ...
                using SqlConnection conn = new SqlConnection(CONNECTION_STRING);
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(sql, conn))
                    {
                        ...
                        while (dr.Read())
                        {
                            ...
                            if (department.Id > 2)
                                employees.Add(new Employee(Convert.ToInt32(dr[0]), dr[1].ToString(), dr[2].ToString(), department, dr[4].ToString(), dr[5].ToString(), dr[6].ToString(), Convert.ToDecimal(dr[7]), Convert.ToDateTime(dr[8]), Convert.ToInt32(dr[9])));
                            else
            ...
            try
            {
                ...
                using SqlConnection conn = new SqlConnection(CONNECTION_STRING);
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(sql, conn))
                    {
                        ...
                        {
                            Department department = _departmentController.Get(Convert.ToInt32(dr[3]));
                            employee = new Employee(Convert.ToInt32(dr[0]), dr[1].ToString(), dr[2].ToString(), department, dr[4].ToString(), dr[5].ToString(), dr[6].ToString(), Convert.ToDecimal(dr[7]), Convert.ToDateTime(dr[8]), Convert.ToInt32(dr[9]));
                        }
                    }
        ...
        public bool Update(Employee employee)
        {
            try
            {
                using SqlConnection conn = new SqlConnection(CONNECTION_STRING);
                {
                    string sql = "UPDATE Employee SET Username = @username, Password = @password, DepartmentID = @departmentID, Name = @name, Email = @email, Phone = @phone, Shifts = @shifts,, Salary = @salary, HireDate = @hiredate WHERE Id = @id";
